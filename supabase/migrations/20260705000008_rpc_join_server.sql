-- Creates a kingdom (plus starter buildings and zeroed troop rows) for the
-- calling user on the given server, in a single transaction.
...
begin
  ...
  insert into kingdoms (server_id, owner_id, name)
  values (p_server_id, auth.uid(), trim(p_kingdom_name))
